...
class GrocyApi {
    ...
    private string $grocyRestURL;
    private string $grocyApiKey;
    private string $answerHttpStatus;
    ...
    {
      return json_decode($this->request(self::GET_REQUEST,"stock",'?', true));
    }

    public function getRecipies()
    {
      return json_decode($this->request(self::GET_REQUEST,"objects/recipes",'?query%5B%5D=type%3Dnormal', true));
    }

    public function getRecipiesPos($rid)
    {
      return json_decode($this->request(self::GET_REQUEST,"objects/recipes_pos",'?query%5B%5D=recipe_id%3D'.$rid, true));
    }
  ...
  */
  private function request($type,$request, $parameter, $customHeader = false)
  {
    if(empty($type)){
      $type = self::GET_REQUEST;
    }
